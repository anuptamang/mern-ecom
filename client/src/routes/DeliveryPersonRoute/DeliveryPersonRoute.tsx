...
function DeliveryPersonRoute({ children }: IProps): JSX.Element | null {
  const auth = useAuth();
  ...
  // If not authenticated or not a delivery person, don't render children (redirect will happen)
  if (auth?.tokenStatus === 'not set' || (auth?.tokenStatus === 'valid' && auth?.result?.role !== 'delivery_person')) {
    return null;
  }
